...
export function About({ config }: AboutProps) {
  ...
  return (
    ...
    >
      ...
      <div className="mx-auto grid w-full max-w-5xl items-center gap-8 py-8 md:grid-cols-2 md:gap-12">
        {config.about.image &&
         config.about.image !== "uploaded" &&
         config.about.image.trim() !== "" &&
         (config.about.image.startsWith("data:") ||
          config.about.image.startsWith("http://") ||
          config.about.image.startsWith("https://") ||
          config.about.image.startsWith("/")) && (
